...
NEURON {
        ...
        RANGE gnabar, gkbar, gl, el, gna, gk, gbar
        : `GLOBAL minf` will be replaced with `RANGE minf` if CoreNEURON enabled
        GLOBAL minf, hinf, ninf, mtau, htau, ntau, sinf, stau
...
ASSIGNED {
	...
	gk (S/cm2) 
       ina (mA/cm2)
        ...
        im (mA/cm2)
        minf hinf ninf sinf
	mtau (ms) htau (ms) ntau (ms) stau (ms)       
}
...
PROCEDURE rates(v(mV)) {  :Computes rate and other constants at current v.
        ...
        TABLE minf, mtau, hinf, htau, ninf, ntau, stau, sinf DEPEND celsius FROM -100 TO 100 WITH 200
...
UNITSOFF
	...
	sum = alpha + beta
        ...
        :"s" adaptive activation system
        sinf = 1/(1+exp(-(v+35)/10))
        stau =  1000/(3.3* (exp((v+35)/40) + exp(-(v+35)/20)))
}
